...
{
	[Serializable]
	public class InternalUploadContext : UploadContext
	{
		...
		internal static InternalUploadContext FindByIDAllServers(string postBackID)
		{
			InternalUploadContext uploadContext = FindByID(postBackID);
            if (uploadContext == null && HttpContext.Current.Session == null)
			{
				uploadContext = new InternalUploadContext();
		...
		internal static InternalUploadContext FindByID(string postBackID)
		{
			if (postBackID == null)
			{
				return null;
			...
			InternalUploadContext uploadContext = HttpContext.Current.Session[InternalUploadContext.NamePrefix + postBackID] as InternalUploadContext;
			if (log.IsDebugEnabled) log.Debug("Session[" + InternalUploadContext.NamePrefix + postBackID + "] = " + uploadContext);
			return uploadContext;
		}
		
		internal static InternalUploadContext FindByIDInAppState(string postBackID)
		{
			InternalUploadContext uploadContext = HttpContext.Current.Application[InternalUploadContext.NamePrefix + postBackID] as InternalUploadContext;
			if (log.IsDebugEnabled) log.Debug("Application[" + InternalUploadContext.NamePrefix + postBackID + "] = " + uploadContext);
			if (uploadContext == null)
			{
				uploadContext = HttpContext.Current.Cache[InternalUploadContext.NamePrefix + postBackID] as InternalUploadContext;
			}
			return uploadContext;
		}
		
		internal InternalUploadContext()
		{
		...
		internal void RegisterPostBack(string postBackID)
		{
			...
			if (HttpContext.Current != null)
			{
				if (HttpContext.Current.Session != null)
				{
					if (log.IsDebugEnabled) log.Debug("Storing UploadContext in Session[" + InternalUploadContext.NamePrefix + PostBackID + "] for SessionID=" + HttpContext.Current.Session.SessionID);
					HttpContext.Current.Session[InternalUploadContext.NamePrefix + PostBackID] = this;
				}
				else
				{
					if (log.IsDebugEnabled) log.Debug("Storing UploadContext in Application[" + InternalUploadContext.NamePrefix + PostBackID + "]");
					HttpContext.Current.Application[InternalUploadContext.NamePrefix + PostBackID] = this;
				}
			}
		}
		...
		internal void SyncWithSession(HttpSessionState session)
		{
			...
			InternalUploadContext ctxInSession = FindByID(PostBackID);
			...
			{
				ctxInSession = new InternalUploadContext();
			}
		...
		internal void CompleteRequest()
		{
			if (!IsAsyncRequest)
			{
				...
				if (ctx != null)
				{
					ctx.Cache.Insert(InternalUploadContext.NamePrefix + PostBackID, this, null, 
									System.Web.Caching.Cache.NoAbsoluteExpiration, 
									TimeSpan.FromSeconds(60));
					if (ctx.Application[InternalUploadContext.NamePrefix + PostBackID] != null)
					{
						ctx.Application.Remove(InternalUploadContext.NamePrefix + PostBackID);
					}
				}
