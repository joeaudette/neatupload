...
namespace Brettle.Web.NeatUpload
{
	internal class FilteringWorkerRequest : DecoratedWorkerRequest
	{
		...
		public FilteringWorkerRequest (HttpWorkerRequest origWorker) : base(origWorker)
		{
			...
			// and has NumAsyncFilesRemaining > -1.  Then start with a copy of that UploadContext.
			if (UploadHttpModule.GetAsyncControlIDFromQueryString(qs) != null)
			{
				...
				while (uploadContext.AsyncFileSizes == null && origWorker.IsClientConnected())
				{
...
*/
		...
		{			
			...
			Stream asyncFileSizesStream = null;
			...
			while (CopyUntilBoundary())
			{
				...
				if (asyncFileSizesStream != null)
				{
					asyncFileSizesStream.Seek(0, System.IO.SeekOrigin.Begin);
					StreamReader sr = new System.IO.StreamReader(asyncFileSizesStream);
					string fieldValue = sr.ReadToEnd();
					asyncFileSizesStream = null;
					if (fieldValue.Length > 0)
					{
						string[] asyncFileSizeStrings = fieldValue.Split(' ');
						long[] asyncFileSizes = new long[asyncFileSizeStrings.Length];
						for (int i = 0; i < asyncFileSizes.Length; i++)
						{
							asyncFileSizes[i] = Int64.Parse(asyncFileSizeStrings[i]);
						}
						WaitForAsyncFiles(asyncFileSizes);
					}
				}
				...
				else if (name != null && name.StartsWith(UploadContext.AsyncFileSizesNamePrefix) 
						&& asyncFileSizesStream == null)
				{
					asyncFileSizesStream = outputStream = new System.IO.MemoryStream();
					readPos = parsePos; // Skip past the boundary and headers
				}
				else if (name != null
		...
		private void WaitForAsyncFiles(long[] asyncFileSizes)
		{
			uploadContext.AsyncFileSizes = asyncFileSizes;
			long asyncBytesTotal = 0;
			for (int i = 0; i < asyncFileSizes.Length; i++)
			{
				asyncBytesTotal += asyncFileSizes[i];
			}
			uploadContext.AsyncBytesTotal = asyncBytesTotal;
			SyncUploadContextWithSession();
			while (IsClientConnected() && uploadContext.NumAsyncFilesReceived < uploadContext.AsyncFileSizes.Length)
			{
				System.Threading.Thread.Sleep(1000);
				UploadHttpModule.AccessSession(new SessionAccessCallback(uploadContext.SyncFromSession));
			}
