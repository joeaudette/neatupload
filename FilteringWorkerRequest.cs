	...
	internal class FilteringWorkerRequest : DecoratedWorkerRequest
	{
		...
		public FilteringWorkerRequest (HttpWorkerRequest origWorker) : base(origWorker)
		{
			...
			if (UploadHttpModule.GetAsyncControlIDFromQueryString(qs) != null)
			{
				...
				while (uploadContext.NumAsyncFilesRemaining == -1 && origWorker.IsClientConnected())
				{
...
*/
		...
		{			
			...
			Stream numAsyncFilesStream = null;
			...
			while (CopyUntilBoundary())
			{
				...
				// If we were receiving the value generated by a MultiFile's hidden NumAsyncFiles field, 
				// wait for the specified number of files
				if (numAsyncFilesStream != null)
				{
					numAsyncFilesStream.Seek(0, System.IO.SeekOrigin.Begin);
					StreamReader sr = new System.IO.StreamReader(numAsyncFilesStream);
					int numAsyncFiles = Int32.Parse(sr.ReadToEnd());
					numAsyncFilesStream = null;
					WaitForAsyncFiles(numAsyncFiles);
				}
				...
				else if (name != null && name.StartsWith(UploadContext.NumAsyncFilesNamePrefix) 
						&& numAsyncFilesStream == null)
				{
					numAsyncFilesStream = outputStream = new System.IO.MemoryStream();
					readPos = parsePos; // Skip past the boundary and headers
				}
				else if (name != null
		...
		private void WaitForAsyncFiles(int numAsyncFiles)
		{
			uploadContext.NumAsyncFilesRemaining = numAsyncFiles;
			SyncUploadContextWithSession();
			while (IsClientConnected() && uploadContext.NumAsyncFilesRemaining > 0)
			{
				System.Threading.Thread.Sleep(1000);
				UploadHttpModule.AccessSession(new SessionAccessCallback(uploadContext.SyncFromSession));
			}
