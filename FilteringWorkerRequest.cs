	...
	internal class FilteringWorkerRequest : DecoratedWorkerRequest
	{
		...
		public FilteringWorkerRequest (HttpWorkerRequest origWorker) : base(origWorker)
		{
			...
			{				
				...
				uploadContext.FileSizes = UploadHttpModule.GetFileSizesFromQueryString(qs);
...
*/
		...
		{			
			...
			while (CopyUntilBoundary())
			{
				...
				// If we were receiving the value generated by the hidden FileSizes field, parse it.
				         ...
				         && null != (controlID = translator.FileFieldNameToControlID(name)))
				{
					...
					if (!uploadContext.IsAsyncRequest && uploadContext.AsyncBytesTotal > 0
